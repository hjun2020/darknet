...
data load_data_enhence_ycbcr(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure, int espcn_scale)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        image im = extract_luminance(random_paths[i], 3);
        im_w = im.w;
        im_h = im.h;
        im_c = im.c;
        if (!im.data) {
        ...
        image sized_truth = make_empty_image(truth_w, truth_h, 1);
        if(w_len != truth_w || h_len != truth_h){
            // sized_truth = resize_image(load_partial_data(data, 3, 0, w_len, 0, h_len, im_w,im_h,im_c), truth_w, truth_h);
            image temp = make_empty_image(w_len, h_len, 1);
            temp.data = load_partial_data(im.data, 0, 0, h_len, w_len, 1, im_h, im_w);
            sized_truth = resize_image(temp, truth_w,truth_h);
            // sized_truth.data = load_partial_data(im.data, 0, 0, truth_h, truth_w, 1, im_h, im_w);
            // printf("WARNING!!!!!!!!!!!!!!!!!!!\n\n\n");

        } else {
            // sized_truth = load_partial_image_stb(data, 3, w_start, w_len, h_start, h_len, im_w,im_h,im_c);
            sized_truth.data = load_partial_data(im.data, h_start, w_start, truth_h, truth_w, 1, im_h, im_w);
